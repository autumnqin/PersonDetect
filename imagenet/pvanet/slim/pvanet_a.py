    ...
    with tf.name_scope(scope, 'pvanet_9', [inputs]):
        with scopes.arg_scope([ops.conv2d, ops.fc, ops.batch_norm, ops.dropout],
            ...
            with scopes.arg_scope([ops.conv2d, ops.max_pool, ops.avg_pool],
                ...
                ###--------------------------------------Conv2-------------------------------------###
                net = ops.conv2d(net, 24, [1, 1], scope='conv2_1/1')
                net = pva_neg_blk(net, 24, [3, 3], scope='conv2_1/2')
                net = ops.conv2d(net, 64, [1, 1], activation=None, scope='conv2_1/3')
                proj = ops.conv2d(end_points['pool1'], 64, [1, 1], activation=None, scope='conv2_1/proj')
                net = tf.add(net, proj, name='conv2_1')
                # 48 x 48 x 64
                end_points['conv2_1'] = net

                net = ops.conv2d(net, 24, [1, 1], scope='conv2_2/1')
                net = pva_neg_blk(net, 24, [3, 3], scope='conv2_2/2')
                net = ops.conv2d(net, 64, [1, 1], activation=None, scope='conv2_2/3')
                net = tf.add(net, end_points['conv2_1'], name='conv2_2')
                # 48 x 48 x 64
                end_points['conv2_2'] = net

                net = ops.conv2d(net, 24, [1, 1], scope='conv2_3/1')
                net = pva_neg_blk(net, 24, [3, 3], scope='conv2_3/2')
                net = ops.conv2d(net, 64, [1, 1], activation=None, scope='conv2_3/3')
                ...
                net = ops.conv2d(net, 128, [1, 1], activation=None, scope='conv3_1/3')
                proj = ops.conv2d(end_points['conv2_3'], 128, [1, 1], stride=2, activation=None, scope='conv3_1/proj')
                net = tf.add(net, proj, name='conv3_1')
                # 24 x 24 x 128
                end_points['conv3_1'] = net

                net = ops.conv2d(net, 48, [1, 1], scope='conv3_2/1')
                net = pva_neg_blk(net, 48, [3, 3], scope='conv3_2/2')
                net = ops.conv2d(net, 128, [1, 1], activation=None, scope='conv3_2/3')
                net = tf.add(net, end_points['conv3_1'], name='conv3_2')
                # 24 x 24 x 128
                end_points['conv3_2'] = net

                net = ops.conv2d(net, 48, [1, 1], scope='conv3_3/1')
                net = pva_neg_blk(net, 48, [3, 3], scope='conv3_3/2')
                net = ops.conv2d(net, 128, [1, 1], activation=None, scope='conv3_3/3')
                net = tf.add(net, end_points['conv3_2'], name='conv3_3')
                # 24 x 24 x 128
                end_points['conv3_3'] = net

                net = ops.conv2d(net, 48, [1, 1], scope='conv3_4/1')
                net = pva_neg_blk(net, 48, [3, 3], scope='conv3_4/2')
                net = ops.conv2d(net, 128, [1, 1], activation=None, scope='conv3_4/3')
                ...
                with tf.variable_scope('conv4_1'):
                    proj = ops.conv2d(net, 256, [1, 1], stride=2, activation=None, scope='proj')
                    with tf.variable_scope('incep'):
                        branch1x1 = ops.conv2d(net, 64, [1, 1], stride=2, scope='0')
                        branch3x3 = ops.conv2d(net, 48, [1, 1], stride=2, scope='1_reduce')
                        branch3x3 = ops.conv2d(branch3x3, 128, [3, 3], scope='1_0')
                        branch5x5 = ops.conv2d(net, 24, [1, 1], stride=2, scope='2_reduce')
                        branch5x5 = ops.conv2d(branch5x5, 48, [3, 3], scope='2_0')
                        branch5x5 = ops.conv2d(branch5x5, 48, [3, 3], scope='2_1')
                        pool = ops.max_pool(net, [3, 3], stride=2, scope='pool')
                        pool = ops.conv2d(pool, 128, [1, 1], scope='poolproj')
                        net = tf.concat(axis=3, values=[branch1x1, branch3x3, branch5x5, pool])
                    net = ops.conv2d(net, 256, [1, 1], activation=None, scope='out')
                    net = tf.add(net, proj)
                    # 12 x 12 x 256
                ...
                with tf.variable_scope('conv4_2'):
                    with tf.variable_scope('incep'):
                        branch1x1 = ops.conv2d(net, 64, [1, 1], scope='0')
                        branch3x3 = ops.conv2d(net, 64, [1, 1], scope='1_reduce')
                        branch3x3 = ops.conv2d(branch3x3, 128, [3, 3], scope='1_0')
                        branch5x5 = ops.conv2d(net, 24, [1, 1], scope='2_reduce')
                        branch5x5 = ops.conv2d(branch5x5, 48, [3, 3], scope='2_0')
                        branch5x5 = ops.conv2d(branch5x5, 48, [3, 3], scope='2_1')
                ...
                with tf.variable_scope('conv5_1'):
                    proj = ops.conv2d(net, 384, [1, 1], stride=2, activation=None, scope='proj')
                    with tf.variable_scope('incep'):
                        branch1x1 = ops.conv2d(net, 64, [1, 1], stride=2, scope='0')
                        branch3x3 = ops.conv2d(net, 96, [1, 1], stride=2, scope='1_reduce')
                        branch3x3 = ops.conv2d(branch3x3, 192, [3, 3], scope='1_0')
                        branch5x5 = ops.conv2d(net, 32, [1, 1], stride=2, scope='2_reduce')
                        branch5x5 = ops.conv2d(branch5x5, 64, [3, 3], scope='2_0')
                        branch5x5 = ops.conv2d(branch5x5, 64, [3, 3], scope='2_1')
                        pool = ops.max_pool(net, [3, 3], stride=2, scope='pool')
                        pool = ops.conv2d(pool, 128, [1, 1], scope='poolproj')
                        net = tf.concat(axis=3, values=[branch1x1, branch3x3, branch5x5, pool])
                    net = ops.conv2d(net, 384, [1, 1], activation=None, scope='out')
                    net = tf.add(net, proj)
                    # 6 x 6 x 384
                ...
                with tf.variable_scope('conv5_2'):
                    with tf.variable_scope('incep'):
                        branch1x1 = ops.conv2d(net, 64, [1, 1], scope='0')
                        branch3x3 = ops.conv2d(net, 96, [1, 1], scope='1_reduce')
                        branch3x3 = ops.conv2d(branch3x3, 192, [3, 3], scope='1_0')
                        branch5x5 = ops.conv2d(net, 32, [1, 1], scope='2_reduce')
                        branch5x5 = ops.conv2d(branch5x5, 64, [3, 3], scope='2_0')
                        branch5x5 = ops.conv2d(branch5x5, 64, [3, 3], scope='2_1')
                        net = tf.concat(axis=3, values=[branch1x1, branch3x3, branch5x5])
                    net = ops.conv2d(net, 384, [1, 1], activation=None, scope='out')
...
def pva_neg_blk(input,
    ...
    with tf.variable_scope(scope):
        conv = ops.conv2d(input, num_filters_out, kernel_size, stride=stride, scope='conv', batch_norm_params=None, activation=None)
        ...
        if scale:
            ...
            alpha = tf.get_variable('scale/alpha', shape=[out_num,], dtype=tf.float32,
                                    initializer=tf.constant_initializer(1.0), trainable=True, regularizer=losses.l2_regularizer(0.00001))
            beta = tf.get_variable('scale/beta', shape=[out_num, ], dtype=tf.float32,
                                    initializer=tf.constant_initializer(0.0), trainable=True, regularizer=losses.l2_regularizer(0.00001))
