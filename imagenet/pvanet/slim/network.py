...
class Network(object):
    ...
    def conv(self, input, k_h, k_w, c_o, s_h, s_w, name, biased=True,relu=True, padding=DEFAULT_PADDING, trainable=True):
        ...
        with tf.variable_scope(name) as scope:
            ...
            #init_weights = tf.contrib.layers.variance_scaling_initializer(factor=0.001, mode='FAN_AVG', uniform=True)
...
#                               regularizer=self.l2_regularizer(0.00001))
#            bn = tf.add(tf.multiply(bn, alpha), beta)
#            bn = tf.nn.dropout(bn, 0.8, name='dropout')
            bn = tf.nn.dropout(input, 0.5, name='dropout')
            bn = tf.nn.relu(bn, name='relu')
            return bn
    ...
    def pva_inception_res_stack(self, input, c_in, name, block_start = False, type = 'a'):
        ...
        with tf.variable_scope(name+'/incep') as scope:
            ...
            ### Add by QQP Start
            bn_scale = tf.nn.relu(bn_scale, name='relu')
            ### Add by QQP End
            ## 1 x 1
            ...
            conv = self.conv._original(self, bn_scale, 1, 1, c_1, stride, stride, name='1_reduce/conv', biased = False, relu=False)
